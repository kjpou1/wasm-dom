...
mono_wasm_invoke_js (MonoString *str, int *is_exception)
{
 ...
 static MonoString*
 mono_wasm_invoke_eval_hook (MonoString *str, int *is_exception)
 {
	DBG("mono_wasm_invoke_eval_hook");

 	if (str == NULL)
 		return NULL;

 	char *native_val = mono_string_to_utf8 (str);
	char *native_res = (char*)mono_wasm_eval_hook((int)native_val, (int)is_exception);
...
void add_internal_calls()
{
 ...
 {
	if (HasEnvironmentVariable("MONO_WASM_DEBUG"))
	{
		debugMode = TRUE;
	}
	...
	if (enable_debugging)
	{
		DBG("mono_wasm_enable_debugging");
